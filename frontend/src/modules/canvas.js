...
export function setupGrafcetCanvas(containerId) {
  ...
  function drawGrafcetSteps() {
    ...
    canvasState.steps.forEach((step) => {
      ...
      const isS0 = step.name === "S0" || step.name === "1.00";
    ...
    const layoutStageWidth = Math.max(containerElement?.clientWidth ?? 800, requiredWidth + 160, 520);
    ...
    canvasState.steps.forEach((step) => {
      ...
      if (!isS0) {
        ...
        const connectingLine = new Konva.Line({
          points: [
            pos.x + stateWidth, pos.y + pos.height / 2,
            actionPanelX, pos.y + pos.height / 2
          ],
          stroke: "#ffffff44",
          strokeWidth: 2,
        });
    ...
    canvasState.steps.forEach((step) => {
      ...
      const loops = [];
      (step.transitions ?? []).forEach((transition) => {
        const targetStep = stepLookup.get(transition.target);
        const targetLevel = targetStep ? (Number.isFinite(targetStep.level) ? targetStep.level : 0) : 0;
  ...
  function notifyModelChange() {
    ...
    const snapshot = canvasState.steps.map(s => ({...s}));
  ...
  function buildLoopPoints(startX, startY, targetX, targetY, loopOffset = 0, manualX = null) {
    ...
    return [startX, startY, startX, startY + 20, marginX, startY + 20, marginX, targetY - 20, targetX, targetY - 20, targetX, targetY];
  }

  return {
    renderSteps: (steps = []) => {
      canvasState.steps = steps.map(normalizeStep);
      drawGrafcetSteps();
    },
    getState: () => ({
      steps: canvasState.steps.map(s => ({...s})),
    }),
    subscribe: (listener) => {
